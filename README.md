...
This repository is an in-progress production foundation: it contains the public experience, membership application-readiness UI, dashboard previews, Supabase security migrations, and security controls. Membership and newsletter endpoints deliberately reject submissions before reading data, so the preview cannot be used to collect real membership, payment, identity-document, or subscriber information before the Supabase, storage, authentication, review, and email integrations below are configured and verified.
...
Copy `docs/environment.example` to an untracked `.env.local`, then set the values appropriate for the environment.
...
| `NEXT_PUBLIC_AUTH_ACTIONS_ENABLED` | Account actions are formally approved | Leave `false` until Supabase Auth signup policy, SMTP, templates, and redirect URLs are reviewed. This UI flag does not disable Supabase Auth REST endpoints. |
...
| `BREVO_API_KEY` | Transactional or marketing email delivery is implemented | Requires an approved sender/domain in Brevo. |
...
The current repository intentionally does not deliver email, Paystack, or Turnstile requests yet. Supplying a key alone does not activate those integrations.

## Database setup and migrations

The security foundation lives in `supabase/migrations/`. Read [docs/DATABASE.md](docs/DATABASE.md), [docs/RBAC.md](docs/RBAC.md), and [docs/RLS_POLICIES.md](docs/RLS_POLICIES.md) before applying it.
...
Do not run `db push` against production until the migration has been reviewed, a backup/rollback plan exists, and RLS tests are ready. There is no seed command in this repository yet; do not create test members from real personal data.
...
- A Supabase production project, Auth redirect URLs, private Storage buckets, and the first controlled super-administrator bootstrap process. Keep Auth invite-only (or disable public signup and email delivery) until approved SMTP, templates, and account-operation controls are in place; only then consider setting `NEXT_PUBLIC_AUTH_ACTIONS_ENABLED=true`.
...
- A delivery provider and approved sender identity for email, plus templates and unsubscribe handling for marketing email.
- Cloudflare Turnstile site and secret keys if public forms are opened.
- A monitoring DSN and alert ownership if Sentry is enabled.
- Verified RLS, privilege-escalation, file-upload, and cross-account tests using non-production test data.

See [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md), [docs/EMAIL.md](docs/EMAIL.md), [docs/TESTING.md](docs/TESTING.md), and [SECURITY.md](SECURITY.md) for the release procedure.
