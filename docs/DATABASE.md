...
The initial Supabase schema is stored in:
...
## Core relationships
    ...
    profiles --< membership_applications --< payments
    membership_applications --1:1-- memberships
...
JSON. Store uploads under:

    <auth-user-id>/<application-or-payment-id>/<filename>
...
payment, membership, notification, or administration data must have RLS enabled
in the same change and tests for both allowed and denied access.
